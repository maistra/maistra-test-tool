...
func TestEgressGateways(t *testing.T) {
	...
	util.Shell("kubectl patch -n %s %s/%s --type merge -p '{\"spec\":{\"istio\":{\"global\":{\"proxy\":{\"accessLogFile\":\"/dev/stdout\"}}}}}'", meshNamespace, smcpAPI, smcpName)

	log.Info("Enable istio-ingressgateway ior")
	util.Shell("kubectl patch -n %s %s/%s --type merge -p '{\"spec\":{\"istio\":{\"gateways\":{\"istio-ingressgateway\":{\"ior_enabled\":true}}}}}'", meshNamespace, smcpAPI, smcpName)
