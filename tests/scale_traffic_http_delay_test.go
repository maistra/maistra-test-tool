...
func cleanupScaleDelay(namespace string) {
	log.Info("# Cleanup ...")
	util.KubeDelete(namespace, bookinfoAllv1Yaml, kubeconfig)
	cleanBookinfo(namespace)
	util.ShellMuteOutput("../scripts/clean_memebers_200.sh")
	time.Sleep(time.Duration(waitTime*2) * time.Second)
}

func TestScaleDelay(t *testing.T) {
	defer cleanupScaleDelay(testNamespace)
	...
	log.Info("# Create 200 members in SMMR")
	util.ShellMuteOutput("../scripts/create_memebers_200.sh")
