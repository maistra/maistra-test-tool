...
func cleanupAccessExternalServices(namespace string) {
	...
	util.Shell(`kubectl get configmap istio -n %s -o yaml | sed 's/mode: REGISTRY_ONLY/mode: ALLOW_ANY/g' | kubectl replace --force -n %s -f -`, meshNamespace, meshNamespace)
...
func TestAccessExternalServices(t *testing.T) {
	...
	t.Run("TrafficManagement_egress_controlled_access_to_external_httpbin_services", func(t *testing.T) {
		...
		util.Shell(`kubectl get configmap istio -n %s -o yaml | sed 's/mode: ALLOW_ANY/mode: REGISTRY_ONLY/g' | kubectl replace --force -n %s -f -`, meshNamespace, meshNamespace)
