...
func TestBookinfo(t *testing.T) {
	...
	log.Log.Info("Test Bookinfo Installation")
	app := examples.Bookinfo{"bookinfo"}
	app.Install(false)

	log.Log.Info("Check pods running 2/2 ready")
	msg, _ := util.Shell(`oc get pods -n bookinfo`)
	if strings.Contains(msg, "2/2") {
		log.Log.Info("Success. proxy container is running.")
	} else {
		t.Error("Error. proxy container is not running.")
	}

	log.Log.Info("Check istiod pod is ready and print istiod logs")
	mesg, _ := util.Shell(`oc get pods -n istio-system | grep istiod`)
	if strings.Contains(mesg, "1/1") {
		log.Log.Info("Success. istiod pod is running with below logs:")
		util.Shell(`oc logs -n %s -l app=istiod | grep info`, meshNamespace)
	} else {
		t.Error("Error. istiod pod is not running.")
	}

	log.Log.Info("Check if bookinfo productpage is running")
	...
	if strings.Contains(mes, "200") {
		log.Log.Info("Success. bookinfo productpage is running")
	} else {
		t.Error("Error. bookinfo productpage is not running.")
		log.Log.Error("Error. bookinfo productpage is not running.")
	}
}
